...
HELP='
NAME
  batch_se_trim_galore.sh - run trim_galore iteratively on a set of SE samples

SYNOPSIS
  batch_se_trim_galore.sh -s -o [-n -a -I -N -S -j -q -O -m -M -t]
  batch_se_trim_galore.sh -h

DESCRIPTION
  Run trim_galore on a set of SE samples.
  ...
  -a  Adapter sequence to be trimmed (passed as -a to trim_galore).
  ...
  -S  Adapter sequence to be trimmed is the first 12bp of the Illumina Small RNA 3p adapter 'TGGAATTCTCGG'.
...
while getopts ":hs:o:n:a:INSj:q:O:m:M:t" opt
do
  case $opt in
    h) printf "$HELP"
    exit 0
...
# => Determine if:
# - user provided actual adapter sequence (-a)
# - user provided an adapter type option (--illumina or --nextera or --small_rna)
# - adapter type should be auto-detected (no option)
# => Add user-provided options to trim_galore's arguments when necessary
if [[ -v ADAPTR ]]
then
  ADAPT_MODE="SEQUENCE PROVIDED BY USER"
  TRIM_GALORE_ARGS+=(-a $ADAPTR)
fi
...
if [[ $ADAPT_MODE == "SEQUENCE PROVIDED BY USER" ]] && ! [[ $TOTAL_INS -eq 0 ]]
then
  printf "Please provide either adapter sequence or one of \"-I\", \"-N\" or \"-S\", not both.\n"
  exit 1
elif ! [[ $ADAPT_MODE == "SEQUENCE PROVIDED BY USER" ]] && [[ $TOTAL_INS -gt 1 ]]
then
  printf "Please provide only one of \"-I\", \"-N\" or \"-S\", not several.\n"
  exit 1
elif ! [[ $ADAPT_MODE == "SEQUENCE PROVIDED BY USER" ]] && [[ $TOTAL_INS -eq 1 ]]
then
  ADAPT_MODE="OPTION \"$ILLUMINA$NEXTERA$SMALL_RNA\" PROVIDED BY USER"
  TRIM_GALORE_ARGS+=($ILLUMINA$NEXTERA$SMALL_RNA)
elif ! [[ $ADAPT_MODE == "SEQUENCE PROVIDED BY USER" ]] && [[ $TOTAL_INS -eq 0 ]]
then
  ADAPT_MODE="AUTO-DETECTION"
fi

# => Add systematic arguments to trim_galore's arguments
TRIM_GALORE_ARGS+=(--quality $QUALITY --stringency $STRINGENCY --length $LENGTH --cores $THREADS)
